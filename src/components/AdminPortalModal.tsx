...
import { Shield, Key, CheckCircle, Scissors, X } from 'lucide-react';
...
export default function AdminPortalModal({ isOpen, onClose }: AdminPortalModalProps) {
  ...
  const handleVerify = (e: React.FormEvent) => {
    e.preventDefault();
    if (passkey.trim().toLowerCase() === 'diamond2026') {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="relative w-full max-w-lg bg-[#0E0F12] border border-[#22252B] rounded-2xl p-6 sm:p-8 shadow-2xl text-stone-100">
        <button
          onClick={onClose}
        ...
        {!isAuthenticated ? (
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-xl font-light tracking-tight text-white">Diamond Cuts Salon Floor & Chair OS</h3>
                <p className="text-xs text-stone-400 font-mono">Master Barber & Dispatch Governance Door</p>
              </div>
            </div>
            ...
            <form onSubmit={handleVerify} className="space-y-4">
              <div>
                ...
                {error && (
                  <p className="text-xs text-red-400 mt-2 font-mono">Invalid barber passkey. Try: diamond2026</p>
                )}
              </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 text-emerald-400 border-b border-stone-800 pb-4">
              ...
              <div>
                <h4 className="text-base font-medium text-white">Chair Floor Manager Authenticated</h4>
                <p className="text-xs text-stone-400 font-mono">Passkey: diamond2026 • Clearance Level: Master Barber</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-left font-mono text-xs">
              <div className="p-3 bg-stone-900/50 rounded-xl border border-stone-800">
                <span className="text-stone-500 block uppercase">Chairs Active</span>
                <span className="text-stone-100 font-semibold text-sm">8 Master Stations</span>
              </div>
              <div className="p-3 bg-stone-900/50 rounded-xl border border-stone-800">
                <span className="text-stone-500 block uppercase">Queue Status</span>
                <span className="text-amber-400 font-semibold text-sm">4 Clients Waiting</span>
              </div>
              <div className="p-3 bg-stone-900/50 rounded-xl border border-stone-800">
                <span className="text-stone-500 block uppercase">Daily Revenue</span>
                <span className="text-emerald-400 font-semibold text-sm">$2,480.00</span>
              </div>
              <div className="p-3 bg-stone-900/50 rounded-xl border border-stone-800">
